...
# Two disk 4 partitions (efi, boot-extended, swap, root)
# Parameters:
...
#	boot_extended=[ext4]  For keyfile storage
function create_custom_boot_disk_layout() {
    local known_arguments=('+swap' '?type' '?luks' '?root_fs')
    local extra_arguments=()
    declare -A arguments; parse_arguments "$@"

    [[ ${#extra_arguments[@]} -eq 2 ]] \
        || die_trace 1 "Expected exactly two positional arguments (boot device and root device)"
    local boot_device="${extra_arguments[0]}"
    local root_device="${extra_arguments[1]}"
    local size_swap="${arguments[swap]}"
    local type="${arguments[type]:-efi}"
    local use_luks="${arguments[luks]:-true}"
    local root_fs="${arguments[root_fs]:-btrfs}"

    # Boot disk layout (GPT)
    create_gpt new_id="gpt_boot" device="$boot_device"
    create_partition new_id="part_${type}" id="gpt_boot" size=512MiB type="$type"
    create_partition new_id="part_gpg" id="gpt_boot" size=256MiB type=linux
    
    # Root disk layout (GPT)
    create_gpt new_id="gpt_root" device="$root_device"
    [[ $size_swap != "false" ]] \
        && create_partition new_id="part_swap" id="gpt_root" size="$size_swap" type=linux
    ...
    format id="part_gpg" type=ext4 label="gpg_key"
    
    # Encrypt and format root partitions
    if [[ "$use_luks" == "true" ]]; then
        [[ $size_swap != "false" ]] \
            && create_luks new_id="luks_swap" name="swap" id="part_swap"
        create_luks new_id="luks_root" name="root" id="part_root"
    fi

    [[ $size_swap != "false" ]] \
        && format id="${use_luks:+luks_}swap" type=swap label=swap
    format id="${use_luks:+luks_}root" type="$root_fs" label=root
    ...
    [[ $size_swap != "false" ]] \
        && DISK_ID_SWAP="${use_luks:+luks_}swap"
    DISK_ID_ROOT="${use_luks:+luks_}root"
    DISK_ID_ROOT_TYPE="$root_fs"
    ...
    # Add GPG key partition to dracut cmdline
    DISK_DRACUT_CMDLINE+=(
        "rd.luks.key=/gpg_key/luks.key:gpg"
    )
}
