...
function download_stage3() {
    ...
    einfo "Verifying stage3 tarball"
    local verified=false
    
    # Method 1: Verify with GPG signature if available
    if download "$STAGE3_RELEASES/${CURRENT_STAGE3}.asc" "${CURRENT_STAGE3}.asc"; then
        einfo "Verifying with GPG signature"
        if gpg --verify "${CURRENT_STAGE3}.asc"; then
            verified=true
        else
            ewarn "GPG signature verification failed, trying other methods"
        fi
    fi
    
    # Method 2: Verify with SHA512 hash
    if [[ $verified == "false" ]]; then
        einfo "Verifying with SHA512 hash"
        local expected_hash=$(grep -E "${CURRENT_STAGE3}$" "${CURRENT_STAGE3}.DIGESTS" | grep -E '^[0-9a-f]{128}' | head -1 | awk '{print $1}')
        local actual_hash=$(sha512sum "${CURRENT_STAGE3}" | awk '{print $1}')
        
        if [[ "$expected_hash" == "$actual_hash" ]]; then
            verified=true
        else
            die "SHA512 hash verification failed"
        fi
    fi
    ...
    if [[ $verified == "true" ]]; then
        ...
        die "Could not verify stage3 tarball"
    fi
...
function generate_gpg_key() {
    einfo "Generating GPG key for LUKS encryption"
    mkdir -p /etc/keys || die "Could not create keys directory"
    gpg --batch --gen-key <<EOF
Key-Type: RSA
Key-Length: 4096
Subkey-Type: RSA
Subkey-Length: 4096
Name-Real: LUKS Encryption Key
Name-Comment: Auto-generated for LUKS
Name-Email: luks@localhost
Expire-Date: 0
%commit
EOF
    gpg --export-secret-keys -a > /etc/keys/luks.key || die "Could not export GPG key"
    chmod 600 /etc/keys/luks.key
}

function setup_custom_portage() {
    [[ $CUSTOM_PORTAGE_CONFIG != "true" ]] && return

    einfo "Setting up custom Portage configuration"
    
    # Copy custom make.conf
    if [[ -f $CUSTOM_MAKECONF_PATH ]]; then
        cp "$CUSTOM_MAKECONF_PATH" /etc/portage/make.conf || die "Could not copy custom make.conf"
        
        # Update CPU flags
        einfo "Updating CPU_FLAGS_X86 in make.conf"
        CPU_FLAGS=$(cpuid2cpuflags | cut -d' ' -f2-)
        if grep -q "^CPU_FLAGS_X86=" /etc/portage/make.conf; then
            sed -i "s/^CPU_FLAGS_X86=.*/CPU_FLAGS_X86=\"${CPU_FLAGS}\"/" /etc/portage/make.conf || die "Could not update CPU_FLAGS_X86 in make.conf"
        else
            echo "CPU_FLAGS_X86=\"${CPU_FLAGS}\"" >> /etc/portage/make.conf || die "Could not add CPU_FLAGS_X86 to make.conf"
        fi
    fi
    
    # Copy package.use directory
    if [[ -d $CUSTOM_PORTAGE_USE_DIR ]]; then
        cp -r "$CUSTOM_PORTAGE_USE_DIR" /etc/portage/ || die "Could not copy package.use directory"
    fi
    
    # Copy package.accept_keywords directory
    if [[ -d $CUSTOM_PORTAGE_ACCEPT_KEYWORD_DIR ]]; then
        cp -r "$CUSTOM_PORTAGE_ACCEPT_KEYWORD_DIR" /etc/portage/ || die "Could not copy package.accept_keywords directory"
    fi
    
    # Copy package.env file and env directory
    if [[ -f $CUSTOM_PORTAGE_ENV_FILE ]]; then
        cp "$CUSTOM_PORTAGE_ENV_FILE" /etc/portage/ || die "Could not copy package.env file"
    fi
    if [[ -d $CUSTOM_PORTAGE_ENV_DIR ]]; then
        cp -r "$CUSTOM_PORTAGE_ENV_DIR" /etc/portage/ || die "Could not copy env directory"
    fi
}
...
function create_btrfs_subvolumes() {
    ...
    einfo "Creating Btrfs subvolumes"
    for subvol in home var etc tmp; do
        btrfs subvolume create "${mountpoint}/${subvol}" || die "Could not create ${subvol} subvolume"
    done
}

function configure_dracut_for_gpg() {
    einfo "Configuring dracut for GPG auto-unlock"
    
    # Install required packages
    try emerge --verbose app-crypt/gnupg
    
    # Copy GPG key to initramfs
    mkdir -p /etc/dracut.conf.d || die "Could not create dracut.conf.d directory"
    echo 'install_items+=" /etc/keys/luks.key "' > /etc/dracut.conf.d/gpg.conf
    
    # Add dracut modules
    echo 'add_dracutmodules+=" crypt gpg "' >> /etc/dracut.conf.d/gpg.conf
    
    # Configure cmdline
    DISK_DRACUT_CMDLINE+=(
        "rd.luks.key=/luks.key:gpg"
