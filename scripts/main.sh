...
# Modify the install_kernel section:
function install_kernel() {
    # Install vanilla kernel
    einfo "Installing kernel sources and related tools"
    ...
    # Prompt for kernel configuration
    einfo "Please configure your kernel now"
    cd /usr/src/linux || die "Could not cd to /usr/src/linux"
    if ask "Do you want to manually configure the kernel?"; then
        make menuconfig || die "Kernel configuration failed"
    else
        # Minimal configuration for LUKS, Btrfs, etc.
        make defconfig || die "Could not create default config"
    ...
    make modules_install || die "Could not install kernel modules"
    make install || die "Could not install kernel"
    
    # Setup custom portage config
    setup_custom_portage
    
    # Create btrfs subvolumes if using btrfs
    if [[ $USED_BTRFS == "true" ]]; then
        create_btrfs_subvolumes "/mnt/gentoo"
    fi
    
    # Configure dracut for GPG if using GPG key
    if [[ $USE_GPG_KEYFILE == "true" ]]; then
        generate_gpg_key
        configure_dracut_for_gpg
    fi
}
